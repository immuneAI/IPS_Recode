...
gene_expression<-read.table("MELA.txt",row.names=1,header=TRUE, sep="\t", dec = ".",check.names=FALSE)
...
for (i in 1:length(sample_names)) {
	...
	plot_a5<-plot_a4+geom_text(aes(x=0, y=6, label=paste("Immunophenoscore: ",IPS[i],sep="")), angle=0,size=6,vjust=-0.5)+ theme(axis.title=element_blank())
  	plot_a6<-plot_a5+geom_text(aes (x=1, y=6, label=paste("Sample ID: ", sample_names[i], sep ="")), angle=0,size=4,vjust=0.6)+ theme(axis.title=element_blank())
  	plot_a <-plot_a6 + theme(plot.margin=unit(c(0,0,0,0),"mm")) + geom_text(vjust=1.15,hjust=0,aes(x=25.5, y=6,label="\n\n\n\n  MHC: Antigen Processing                                 EC: Effector Cells\n   CP: Checkpoints | Immunomodulators              SC: Suppressor Cells\n\n", hjust = 0), size=4)
